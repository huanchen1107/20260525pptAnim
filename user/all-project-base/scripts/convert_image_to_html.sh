...
SLIDES_DIR="${1:-user/project-1/slides}"
FRAME_RATE=30
...
for slide_dir in "${slide_dirs[@]}"; do
  ...
  if [[ ! -f "$layout_json_file" ]]; then
    ...
    python3 "${script_dir}/../utils/orchestrator.py" "$image_file" "$layout_json_file" || {
  ...
  if [[ -f "$layout_json_file" ]]; then
    ...
    python3 "${script_dir}/../utils/render_html_from_layout.py" "$layout_json_file" "$custom_html_file" "$duration_frames"
  fi
